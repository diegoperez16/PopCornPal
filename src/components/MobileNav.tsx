...
export default function MobileNav() {
  ...
  const postAlertItems = [
    { path: '/profile', icon: User, label: 'Profile' },
  ]

  const renderNavItem = (item: { path: string; icon: React.FC<{ className?: string; strokeWidth?: number }>; label: string }) => {
    ...
    return (
      <button
        ...
        className={`flex flex-col items-center justify-center flex-1 h-full transition-all duration-200 ${
          isActive ? 'text-white' : 'text-gray-500 active:scale-95'
        }`}
      >
  ...
  return (
    <nav className="mobile-nav md:hidden fixed bottom-0 left-0 right-0 bg-gray-900/95 backdrop-blur-lg border-t border-gray-800 z-50 safe-area-bottom">
      <div className="flex items-center justify-around h-16 px-2">
        {preAlertItems.map(renderNavItem)}

        {/* Alerts — full-screen sheet on mobile */}
        ...
        {postAlertItems.map(renderNavItem)}
      </div>
    </nav>
  )
}
